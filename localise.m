...
%     disp(numNearby);
    ...
    turnNoise = normrnd(0,0.8,num,1)./(2*pi);
    moveNoise = normrnd(0,1,num,1);
   
    botSim.turn(turn); %turn the real robot.  
    botSim.move(move); %move the real robot. These movements are recorded for marking 
    for i =1:num %for all the particles. 
        particles(i).turn(turn+turnNoise(i));%turnNoise(i)); %turn the particle in the same way as the real robot
        particles(i).move(move+moveNoise(i));%moveNoise(i)); %move the particle in the same way as the real robot
    end
